 ...
 *
 * 꺼져 있어도 **관리자는 «새 디자인 미리보기»(F23)로 볼 수 있습니다.** 그게 이 값이
 * `false` 인 채로 페이지를 만들어 둔 이유고, 손님에게는 404 입니다.
 *
 * 🔴 **켜는 것은 대표님 결정입니다.** 켜면 주 메뉴에 "제품" 이 서고 검색에도 잡힙니다.
 * 켤 때 같이 할 일: ① `app/sitemap.ts` 가 자동으로 포함합니다(코드 수정 불필요)
 * ② 제품 전용 사진으로 갈아 끼우기(지금은 실적 사진을 다시 쓰는 중입니다).
 *
 * `SHOW_FABRICATION` 과 같은 모양의 스위치지만 **방향이 반대**입니다 —
 * 저건 "사진이 없어서 내린 것", 이건 "아직 안 연 것".
 */
