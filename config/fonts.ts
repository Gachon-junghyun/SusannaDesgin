 ...
 * 기준은 **넷**이 모두 «사용 가능» 인 것만입니다:
 *   - **인쇄** — 간판 출력물
 *   - **BI/CI** — 간판 글자는 대개 손님의 상호명·로고입니다. 여기가 «조건부» 면 뺐습니다
 *   - **웹사이트** — 이 페이지가 그 글꼴로 견본을 그립니다
 *   - 🔴 **임베딩** — 2026-09-24 에 기준으로 «추가»했습니다. 원래 셋만 적혀 있었는데,
 *     **«서버에 폰트 파일을 올려도 되나»를 답하는 칸은 「웹사이트」가 아니라 「임베딩」입니다**
 *     (표의 설명 원문: «웹사이트 및 프로그램 서버 내 폰트 탑재, E-book 제작»).
 *     아래 16종은 `next/font/google` 로 **우리 서버가 파일을 내보내는** 넷이 섞여 있어서
 *     이 칸이 실제로 필요합니다. ✅ **16종 전수 재확인 결과 인쇄·웹사이트·임베딩·BI/CI·영상·포장지가
 *     전부 «사용 가능» 이고 「조건부」·「불가」는 0건입니다**(2026-09-24, 크롬으로 16장 전부 열어 실측).
 *     **글꼴을 새로 넣을 때 이 칸을 빠뜨리지 마세요.**
 ...
 * ✅ **2026-09-24 실측: 12벌 전부 눈누가 적어 둔 주소와 «글자 하나까지» 같습니다**(에이투지체만
 * 퍼센트 인코딩 차이). 이게 중요한 이유: 대법원 98도732 는 **폰트 «파일»**을 컴퓨터프로그램저작물로
 * 보는데, 배포처 주소로 불러 쓰면 **우리가 파일을 복제·전송하지 않습니다.** 반대로 아래 넷
 * (검은고딕·나눔손글씨 붓·본고딕·고운바탕)은 `next/font/google` 이라 **우리 서버가 파일을 내보냅니다** —
 * 그래서 위 「임베딩」 칸이 그 넷의 근거입니다.
 * ⚠️ **나눔손글씨 붓은 파일 안에 라이선스 고지가 없습니다**(2026-09-24 실측: name id0 이
 * «© 2010 NHN Corporation. All rights reserved.», 라이선스 필드 13·14 는 비어 있음). 눈누 표는
 * OFL 인데 파일은 그렇게 안 적혀 있습니다 — **KCC간판체와 같은 «배포처 태그 ↔ 파일 원문» 어긋남**입니다.
 * ⚠️ **OFL 관점에서 `next/font` 의 글자범위 쪼개기는 «수정»입니다**(OFL-FAQ 2.6 원문). 허용되지만
 * 예약이름(RFN)을 못 쓰게 되는데, **넷 중 걸리는 건 0건입니다** — 고운바탕·검은고딕은 RFN 선언이 없고
 * 본고딕의 RFN 은 `'Source'` 라 `Noto Sans KR` 이라는 이름에 안 걸립니다.
 * 🔴 **눈누는 라이선서가 아닙니다**(푸터 원문: «라이센스 문의는 저작권자에게»). 요약표는 «안내»입니다 —
 * **간판을 실제로 제작해 납품할 때는 저작권자 공식 페이지를 보세요.**
