...
### 🔴 2. 구글 검색 — 등록은 됐고, 남은 건 세 가지입니다

**✅ 구글에 "수산나디자인" 을 치면 우리 홈페이지가 1위로 나옵니다** (2026-07-27 확인).
대표님이 07-26 에 등록하신 게 반영됐습니다. 제목·설명도 넣은 그대로 나옵니다.

그런데 **제미나이 같은 AI 는 여전히 홈페이지를 모릅니다.** 이유를 찾아보니
세 가지가 걸려 있었습니다.

#### ✅ (1) `http://` 주소 문제 — 해결됨 (2026-07-27)

구글이 색인한 주소가 `https://` 가 아니라 `http://susannadesign.co.kr` 이었습니다.
Cloudflare 의 **Always Use HTTPS** 를 켜서 해결했습니다. `http` 로 들어와도,
`www` 까지 겹쳐 들어와도 정식 주소 한 곳으로 모이는 것을 확인했습니다.

#### ⚠️ (2) 홈 말고 다른 페이지는 아직 색인이 안 됐습니다

`시공실적` `회사소개` 등 9개 주소가 아직 구글에 안 들어갔습니다.
Search Console → **Sitemaps** 에 `sitemap.xml` 을 제출하고,
**URL 검사**로 `/works` `/about` 에 색인을 요청해 주세요. 절차는
[`docs/SEO.md`](docs/SEO.md) **C4 상세**.

#### ⚠️ (3) AI 가 홈페이지 대신 잡코리아·인스타그램을 인용합니다

제미나이가 회사는 아는데 근거로 채용사이트·기업정보사이트를 답니다.
**차단 문제가 아닙니다** — AI 크롤러 8종이 전부 정상 통과하는 걸 확인했습니다.
**인용할 만한 사실이 홈페이지에 부족한 것**이 원인입니다.

| 필요한 것 | 지금 상태 |
|---|---|
| **인스타그램 주소** (`instagramUrl`) | ✅ **연결됨** — `@susanna_design541`. 이제 구글이 홈페이지와 인스타그램을 같은 회사로 봅니다 |
| **사업자등록번호** (`bizNo`) | ✅ **입력됨** — 푸터 법정표기 + 구조화 데이터 양쪽에 나갑니다 |
| 시공사진 | 50자리가 아직 "준비 중" (위 1번) |

> 이제 홈페이지가 인스타그램 계정과 사업자등록번호를 **1차 출처로** 갖게 됐습니다.
> 남은 건 시공사진입니다 — AI 든 사람이든 결국 그걸 보고 판단합니다(위 1번).
>
> ⚠️ **반영에는 시간이 걸립니다.** 구글 AI 개요의 근거 자료는 색인보다 몇 주
> 늦게 갱신됩니다. 오늘 고친 것이 내일 제미나이 답변에 나타나지 않는 게 정상입니다.
...
> 신규 스마트플레이스는 **90일간 노출 우대**를 받으니 지금이 가장 효율이 좋습니다.
> 소유확인 코드를 주시면 `config/site.ts` 에 넣어 드립니다.

전체 절차와 등록 후 운영(블로그·리뷰·사진)은 [`docs/SEO.md`](docs/SEO.md) **C 섹션**에 있습니다.

#### 계정 없이 미리 해 둔 것 — 색인 통보 (IndexNow)

```bash
npm run indexnow
```

Bing·네이버 등에 "이 주소들 읽어 가라"고 **즉시 통보**하는 기능을 넣어 뒀습니다
(계정 불필요). **Bing 은 ChatGPT·Perplexity 가 읽는 인덱스**라 AI 인용 경로가
그쪽부터 열립니다. **배포한 뒤 한 번만 실행**하시면 됩니다.

> ⚠️ **구글은 이 방식에 참여하지 않습니다.** 구글만은 위 3번을 꼭 하셔야 합니다.
> 네이버도 위 2번(서치어드바이저) 등록이 끝나야 이 통보가 효과를 냅니다.
...
| 항목 | 키 | 비고 |
|---|---|---|
| ~~사업자등록번호~~ | ~~`bizNo`~~ | ✅ `314-86-35284` 입력 완료 — 푸터 + 구조화 데이터 `taxID` |
