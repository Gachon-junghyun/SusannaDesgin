...
#### 🔴 (1) `http://` 주소가 `https://` 로 안 넘어갑니다 ← 가장 급합니다

구글이 색인한 주소가 `https://` 가 아니라 **`http://susannadesign.co.kr`** 입니다.
정작 페이지는 "내 정식 주소는 https 다" 라고 말하고 있어 서로 어긋납니다.
같은 사이트가 두 주소로 갈리면 검색엔진 평가가 반으로 쪼개집니다.
`www` 는 막아 뒀는데 `http` 는 빠져 있었습니다.

**고치는 법 — 클릭 세 번, 배포 필요 없음**

1. [dash.cloudflare.com](https://dash.cloudflare.com) → `susannadesign.co.kr`
2. **SSL/TLS** → **Edge Certificates**
3. **Always Use HTTPS** 켜기

> 이건 코드로 하면 안 됩니다. 잘못 걸면 **무한 반복으로 사이트가 죽습니다.**
> 대시보드 토글이 안전하고 즉시 적용됩니다.
...
| 필요한 것 | 지금 상태 |
|---|---|
| **인스타그램 주소** (`config/site.ts` 의 `instagramUrl`) | 비어 있음 → 구글이 홈페이지와 인스타그램을 **다른 회사로 봅니다** |
| **사업자등록번호** (`bizNo`) | 비어 있음 → 비즈노·머니핀은 이걸 갖고 있어서 잡힙니다 |
| 시공사진 | 50자리가 아직 "준비 중" (위 1번) |

> **인스타그램 주소와 사업자등록번호만 알려주시면 바로 넣어 드립니다.**
> 이 둘이 AI 가 "이 홈페이지 = 그 회사" 라고 인식하는 데 가장 큰 신호입니다.
