...
>
> **남은 일 목록은 [`ARCHITECTURE.md`](ARCHITECTURE.md) §7 하나로 모았습니다.**
> 세 문서가 각자 목록을 들고 있으면 반드시 어긋나서, 여기서는 중복하지 않습니다.
>
> 이 문서에만 있는 배포 관련 확인거리는 두 가지입니다.
> - **4단계 Supabase 정지 방지** — GitHub Actions 시크릿 등록 여부를 확인하지 못했습니다.
>   저장소 **Actions** 탭에 "Supabase 깨어 있게 유지" 실행 기록이 초록색인지 봐 주세요
> - **테스트 문의 2건 삭제** — `/admin/quotes` 의 `[검증] 지워주세요` (아래 3단계)
>
