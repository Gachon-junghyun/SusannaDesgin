...
>
> **남은 일 3가지**
> 1. **견적 문의 실접수 테스트** (아래 **3단계**) — 아직 안 된 유일한 필수 항목입니다
> 2. **AI 답변 인용 허용** — Cloudflare 대시보드 → AI Crawl Control 에서 관리형
>    `robots.txt` 끄기. 절차와 트레이드오프는 [`SEO.md`](SEO.md)
> 3. **4단계 Supabase 정지 방지** — GitHub Actions 시크릿 등록 여부를 확인하지 못했습니다.
>    저장소 **Actions** 탭에 "Supabase 깨어 있게 유지" 실행 기록이 초록색인지 봐 주세요
>
> `www` → 비`www` 301 은 **코드로 해결했습니다**(`next.config.ts`) — 대시보드 규칙이
> 필요 없습니다. 배포된 뒤 `www` 주소로 들어가 주소창이 비www 로 바뀌는지만 봐 주세요.
>
...
- [ ] **견적 문의를 실제로 넣어보고**, `/admin/quotes` 에 뜨는가 ★ → **아직 안 함**
- [x] `/admin` 로그인이 되는가 → ✅ 로그인 폼 정상 (Supabase 연결됨)
- [ ] 휴대폰으로도 열어보기 → 대표님 확인 필요

> ### ★ 견적 문의 테스트는 꼭 해 주세요 — 아직 안 된 유일한 필수 항목입니다
>
> [susannadesign.co.kr/quote](https://susannadesign.co.kr/quote) 에서 본인 이름·번호로
> 한 건 넣어 보고, [/admin/quotes](https://susannadesign.co.kr/admin/quotes) 에 뜨는지
> 보세요. 뜨면 확인 처리하고 삭제하면 됩니다.
>
> **이게 안 되면 일감이 새는 겁니다.** 손님은 "접수 완료"를 보는데 회사는 아무것도
> 못 받는 상황이 가장 위험합니다. (테이블은 만들어져 있고 현재 접수 건수는 0건입니다.)
>
> 대신 넣어드리지 않은 이유: 실제 문의함에 가짜 문의를 남기는 일이라 대표님이
> 직접 하시는 게 맞습니다.
