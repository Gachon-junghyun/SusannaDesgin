...
>
> **남은 일**
> 1. **네이버 등록** ★ 가장 급함 — 스마트플레이스·서치어드바이저. 구글은 07-26
>    등록 완료돼 "수산나디자인" 1위로 나옵니다. 네이버가 지역 유입의 8할이고,
>    서치어드바이저를 하면 IndexNow 네이버 403 도 같이 풀립니다. [`SEO.md`](SEO.md) C3
> 2. **구글 색인 확대** — 홈 1페이지만 색인됨. Search Console 에서 사이트맵 제출 +
>    `/works` `/about` 색인 요청. [`SEO.md`](SEO.md) C4 상세
> 3. **`site.bizNo`(사업자등록번호) 입력** — 값을 받으면 푸터에 자동 표기.
>    AI 가 자사 홈 대신 비즈노·머니핀을 인용하는 이유 중 하나 | `config/site.ts`
> 4. **네이버 등록 후 `npm run indexnow` 재실행** — 첫 실행에서 네이버만 403
>    (미등록). Bing·전체중계는 202 접수됨. [`ARCHITECTURE.md`](ARCHITECTURE.md) F17
...
>
> **해결됨**: `www` → 비`www` 301 (코드, 배포 확인) · **`http` → `https` 301
> (Cloudflare Always Use HTTPS, 2026-07-27)** · AI 크롤러 허용 (Cloudflare 기본
> 차단 해제 + 코드 명시 허용) · 운영 화면에서 개발자용 정보 숨김 ·
> **엔티티 연결 `sameAs` 인스타그램 (2026-07-27)**
>
