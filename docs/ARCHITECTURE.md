...
| **P17** | `/fonts` | `app/fonts/page.tsx` | **요청 시 SSR** (F25) | `config/fonts.ts` (눈누 상업용 무료 글꼴 14종) — `SHOW_FONTS` 꺼지면 관리자 외 404 |
...
### F25. 글꼴 견본 (`/fonts`) — 🔴 만들어 뒀지만 아직 «안 연» 페이지 (2026-09-23 신설)

손님이 **가게 이름을 한 번 적으면 글꼴 14종 카드에 한꺼번에 들어가고**, 카드의 「이 글꼴로 견적」이
`/quote?font=<슬러그>` 로 보냅니다. 정현님(개발)이 형제 저장소 세션에서 요청한 것이고 **대표님은 아직 안 봤습니다.**

```
app/fonts/page.tsx          !SHOW_FONTS && !preview.on → notFound()  ·  검은고딕·나눔손글씨붓은 next/font/google(preload:false)
app/fonts/specimen.css      눈누 «웹폰트로 사용» 주소 그대로, 굵기 한 벌씩 (@font-face 11개)
...
- ⚠️ **글꼴은 카드가 화면에 들어올 때 붙입니다**(IntersectionObserver). 14벌 합계가 수 MB 이고 정묵바위체
...
- 켜면 세 곳이 같이 열립니다: 페이지 404 해제 · 주 메뉴 「글꼴」 · `noindexPaths` 에서 빠지며 사이트맵 포함.
...
| 구분 | 내용 | 위치 |
|---|---|---|
| 미검증 | **`/fonts`(F25)를 브라우저로 한 번도 못 봤습니다** (2026-09-23). `lint`·`build` 는 통과했지만 **실제 페이지 화면은 못 봤습니다** — 스위치를 잠깐 켜는 것이 차단돼, 같은 마크업·글꼴·색으로 그린 임시 사본을 1440px·390px 로 찍어 확인했습니다(390px 가로 넘침 0). 관리자 미리보기로 실물을 한 번 보세요 | F25 |
