...
| `0011_signmodel_body.sql` | **간판 종류에 「손님용 설명」 칸** — `content_blocks.body` + 9종 초안 (F24-d). 🔴 **아직 안 돌렸습니다.** 안 돌려도 화면·저장은 정상이지만(관리자 저장이 그 칸만 빼고 재시도) **상세페이지에 설명이 안 나오고 색인도 안 됩니다** |
| `0010_quote_product.sql` | **견적 문의에 「보고 온 제품」 칸** — `quotes.product` 하나 (F24-c). 🔴 **아직 안 돌렸습니다.** 안 돌려도 접수는 정상입니다(API 가 그 칸만 빼고 재시도) — 대신 **어느 제품을 보고 왔는지가 DB 에 안 쌓입니다.** 알림 메일에는 그때도 나갑니다 |
...
| 구분 | 내용 | 위치 |
|---|---|---|
| 🔴 실행 | **`0011_signmodel_body.sql` 을 같이 돌리세요** (2026-09-09). 안 돌리면 제품 상세페이지 9장에 **설명이 한 줄도 안 나오고**, 그 상태에서는 검색에도 안 잡힙니다(일부러 그렇게 뒀습니다 — 내용 없는 페이지를 색인시키지 않으려는 것). 돌린 뒤 `/admin/content?section=sign_model` 에서 **「손님용 설명」을 손보세요** — 지금 들어가는 9개는 `SIGNTYPES.md` 에서 사실만 옮긴 **초안**입니다 | §5 · F24-d |
| 🔴 실행 | **`0010_quote_product.sql` 을 Supabase SQL Editor 에서 돌리세요** (2026-09-09). 안 돌려도 문의는 안 잃지만(API 가 `product` 칸만 빼고 재시도), **「어느 제품을 보고 왔나」가 DB 에 안 쌓입니다** — 알림 메일에는 나가므로 **메일을 안 열면 그냥 사라집니다.** 한 줄짜리 `alter table` 입니다 | §5 · F24-c |
