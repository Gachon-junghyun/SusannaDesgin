...
│                           `SHOW_HOME_FABRICATION = false` (app/page.tsx 맨 위)
...
> | FABRICATION 구역 전체 | `SHOW_HOME_FABRICATION` **스위치로 차단** | 그 값을 `true` 로 |
>
...
> 칸과 "제작 공정" 탭이 계속 보이는데, **사진 칸은 이제 어느 화면에도 안 나갑니다**
> (글 문구는 홈·`/process` 양쪽에 정상 반영됩니다). "제작 공정" 은 `/about#equipment`
> 에 여전히 나갑니다. F19 의 "구역을 비울 수 없다" 함정과 같은 계열이니 헷갈리지 마세요.
