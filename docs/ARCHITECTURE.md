...
| | |
|---|---|
| 최종 갱신 | 2026-09-08 (**`/products` 카드·탭 배치 개편 — F24-b.** 대표님이 렌터카 Figma 템플릿 화면을 주며 "레이아웃만 저 느낌으로, 색이랑 디자인은 수산나 그대로"라고 정했습니다. 가져온 것은 **배치 넷**(가운데 큰 제목 · 알약 탭 줄 · 테두리 3열 카드 · 맨 아래 가로 띠)뿐이고 **팔레트·타이포·정사각 썸네일은 하나도 안 바꿨습니다**. 새 파일은 `components/CatalogTabs.tsx` 하나이고 `SignTypesGrid`·`MaterialsGrid`·`app/products/page.tsx` 를 고쳤습니다. 🔴 **탭 축을 지어내지 않았습니다** — 참고 화면의 «세단·SUV» 자리에 넣을 축이 간판 종류에는 없어서(`eyebrow` 가 T1~T9 일련번호) «이 페이지의 목록 두 개»로 세웠습니다 [P6]. ⚠️ 2026-09-05 에 내렸던 **사양 칩을 잠깐 되살렸다가 같은 날 대표님 지시로 다시 뺐습니다** — «60mm · 벽 이격 60mm» 같은 줄이 손님에게 쓸모없다는 판단이고, **세 번째로 되살리지 마세요**. 격자 아래 문구에서 치수 조항도 같이 빼고 **렌더 고지만 남겼습니다**. ⚠️ `SignTypesGrid`↔`MaterialsGrid` 의 **230×230 짝은 끊었습니다** — 성격이 «파는 카드»와 «견본 조각»으로 갈렸습니다. 검증: `npm run lint`·`npm run build` 통과, `SHOW_PRODUCTS` 를 잠깐 `true` 로 바꿔 헤드리스 크롬으로 1440px·520px 확인 후 `false` 로 되돌렸습니다. **390px 폰 폭·실기기는 못 봤습니다** [P6]) · 2026-09-05 (**`/products` 에 «재질» 평면 목록 추가 — F24-a.** 대표님(=이 리포를 관리하는 사람)이 형제 저장소 `DeGaJa_Agent`(susanna 도메인)에서 먼저 «홈페이지를 쇼핑몰 느낌으로»를 논의하다가, 새 리서치 없이 **이미 있는 F24 제품 카탈로그를 그대로 쓰라**고 정정하며 나온 작업입니다. 재질 10종(화강석·벽돌·콘크리트·파사드 통짜·대리석·금속·녹슨 금속·도장·타일·목재)을 `ProductsGrid` 아래 평면 그리드로 얹었습니다 — **분류 탭 없이 있는 그대로 다 보여줍니다**(재질을 묶을 축이 아직 없어서). 사진은 실사진이 아니라 형제 저장소의 522장 렌더 카탈로그(간판 9종 × 재질 58종, 벽·조명·카메라 고정)에서 고른 대표 10장이고, 화면에 "3D 렌더입니다"라고 밝혔습니다[P6]. **DB 연결은 없습니다** — `products`(F19 `product` 섹션)와 달리 `config/content.ts` 코드가 유일한 원본이라, 재질을 늘리려면 코드 수정 + 배포가 필요합니다. `SHOW_PRODUCTS`·브랜드 청록(`#00a79d`)·정사각 썸네일·가격 없음 등 F24 의 기존 결정은 **하나도 안 바꿨습니다** — 같은 게이트 아래 얹기만 했습니다. 검증: `npm run lint`·`npm run build` 통과, 로컬에서 `SHOW_PRODUCTS` 를 잠깐 `true` 로 바꿔 화면을 확인한 뒤 `false` 로 되돌렸습니다(대표님 결정이라 커밋에 `true` 로 남기지 않습니다)) · 2026-08-17 (**제품 카탈로그 `/products` — F24 신설. 🔴 아직 «안 연» 페이지고 관리자만 미리보기로 봅니다.** 쿠팡처럼 «파는 리스트»를 우리도 만들자는 요청이라, **먼저 네 곳을 실제 크롬으로 열어 DOM 을 쟀습니다**(쿠팡·네이버 가격비교·오늘의집·**아트네온** — 대전 경쟁사) → [`reference/reference.md`](../reference/reference.md) 부록 A. 🔴 **가장 큰 발견: 아트네온은 Cafe24 쇼핑몰인데도 목록에 가격을 안 띄웁니다**(DOM 엔 명목가 `1,000원` 이 있는데 화면엔 없습니다). 맞춤 제작이라 정가가 성립하지 않아서고, 그래서 **우리 카드에도 가격 칸이 없습니다** — 대표님이 «사진 카탈로그» 로 정하셨습니다. 그다음 발견: **네 곳 전부 썸네일이 정사각 1:1** 이고 예외가 하나도 없었습니다 → 제품은 `aspect-square`, 실적은 `aspect-4/3` 으로 **일부러 갈랐습니다**(합치면 제품 목록이 실적 목록처럼 보입니다). 그리고 **쿠팡의 진짜 무기는 카드가 아니라 왼쪽 속성 필터**(가로길이·설치형태·재질·색상)였습니다 — 우리는 «분류» 축 하나(실외·실내·구조물)로 시작했고, **탭은 코드에 고정하지 않고 `eyebrow` 값에서 모읍니다**(고정하면 관리자가 새 분류를 적었을 때 그 제품이 어느 탭에도 안 걸립니다). ⚠️ **별점·리뷰수 칸은 일부러 안 만들었습니다** — 네이버가 그걸 다 넣는 건 숫자가 **있기 때문**이고, 우리는 없어서 만들면 빈 칸이 줄지어 나갑니다(FABRICATION 을 내린 것과 같은 실패). 보이는 조건은 **세 곳이 한 값에서 갈립니다** — 페이지(`SHOW_PRODUCTS \|\| preview.on`, 아니면 **404**) · 주 메뉴 · `noindexPaths`→사이트맵. 🔴 **«링크만 안 걸면 된다» 로 두지 않았습니다** — 주소는 추측 가능하고 `noindex` 는 «색인하지 말라»지 «보여주지 말라»가 아닙니다. ⚠️ 사진 6장은 **실적 사진을 다시 씁니다**(제품 전용 촬영본 없음). 한 장씩 열어 보고 골랐고 그 과정에서 둘을 바꿨습니다 — `work-23`(홈센터)은 계산대 잡동사니와 날짜 스탬프가 찍혀 있었고 `work-19`(자자고호텔)는 후광 채널이라 «채널 간판» 과 겹쳤습니다. **제품 이름도 사진에 맞춰 고쳤습니다**("층별 안내·유도 사인" → "실내 사인·스카시 문자") — 보여줄 수 없는 것을 이름으로 약속하지 않으려는 것입니다 [P6]. 🔴 **`0008_product.sql` 은 아직 안 돌렸습니다** → §7. 실측 확인: 익명 `/products` **404** · 사이트맵에 `products` **0건** · 미리보기 켜면 주 메뉴에 "제품" 이 서고 화면이 그려짐 · 분류 «실내» 누르면 6종 → **2종**) · 2026-08-17 (**새 디자인 미리보기 — F23 신설. 🔴 설계 원칙 A3 에 «유일한 예외»를 명시했습니다.** 같은 주소에서 관리자에게만 새 화면을 보여주고, 왼쪽 아래 막대로 손님 화면과 오가며 비교합니다. 대표님이 새 디자인을 «실제 그 주소에서» 검증하고 싶다고 해서 열었고, **A3 와 정면으로 부딪히는 변경이라 먼저 보고하고 승인을 받았습니다.** A3 가 지키려던 것 중 «정적 생성»은 **이미 깨져 있었고**(DB 를 읽는 다섯 페이지가 `force-dynamic`), 진짜 대가는 **CDN 캐시 경로**입니다 → §7 에 부채 한 줄을 새로 적었습니다: 🔴 **캐시를 켤 때 `susanna-preview` 쿠키를 우회시키지 않으면 대표님이 보던 화면이 캐시에 담겨 손님에게 나갑니다.** 지금은 공개 HTML 이 `no-store` 라 안 터지지만, 두 부채는 **같이 갚아야 하는 한 쌍**입니다. 딸려 나온 것 넷: ① **정적이던 다섯 장이 동적**이 됐습니다(`/support` `/quote` `/privacy` `/terms` `/no-email-collect`) — 루트 레이아웃이 쿠키를 읽으면 그 아래가 전부 동적이 되기 때문이고, 빌드 결과에서 `○` 가 사라진 것으로 확인했습니다. 되돌리려면 `app/layout.tsx` 의 `getPreview()` 한 줄을 빼면 됩니다 ② 🔴 **로그인 만료를 조용히 «꺼짐»으로 삼키지 않습니다**(`PreviewState.stale`) — 토큰은 1시간이면 만료되는데 그때 원래 화면을 그냥 보여주면 「켰는데 안 바뀐다」의 원인이 화면 어디에도 안 남습니다. 겸해서 `SessionKeeper` 를 **`next/dynamic` 으로** 공개 페이지에도 띄워 애초에 잘 안 만료되게 했습니다 — **그냥 `import` 하면 `@supabase/ssr` 이 모든 공개 페이지 번들에 들어갑니다**(조건부로 그려도 그렇습니다) ③ **화면 문구에서 "시안" 을 뺐습니다** — 이 회사는 그 말을 **손님에게 내는 간판 디자인 시안**이라는 뜻으로 홈 PROCESS 에서 이미 쓰고 있어서, 그대로 뒀으면 대표님이 견적 기능으로 읽었을 자리입니다 ④ 막대 색을 `bg-accent text-white` 로 짰다가 되돌렸습니다 — **흰 글자가 주황 위에서 3.14:1** 이라 F12 기준(4.5)에 못 미쳤고, 주황 위 `ink`(5.66:1)로 갔습니다. **산술로만 계산했고 브라우저로는 안 쟀습니다.** 🔴 **크롬으로 끝까지 확인했습니다**(2026-08-17): 관리자 로그인 → 홈에 막대 표시 → 눌러서 켜짐 → `/works` 로 옮겨도 유지 → 다시 눌러 꺼짐. `document.cookie` 에 미리보기 쿠키가 **안 보입니다**(`httpOnly` 확인). 익명 응답에는 막대 마크업·`setPreview`·`SessionKeeper` **전부 0건**. ⚠️ 그 과정에서 **막대가 왼쪽 아래 Next 개발 표시등에 가려** 앞글자가 안 보이는 걸 발견해 `md:bottom-20` 으로 띄웠습니다 — 배포본엔 그 표시등이 없지만 **새 디자인을 보는 시간의 대부분이 개발 서버**입니다. ⚠️ 처음에 "안 보인다" 로 신고가 왔는데 **원인이 코드가 아니라 둘이었습니다** — 아직 배포 전인데 운영 주소를 본 것 + 로컬 로그인 만료. F23 에 확인 순서를 적어 뒀습니다) · 2026-08-14 (**접근성 정비 — F12 에 세 절 신설. 색은 재보기만 하고 안 바꿨습니다.** UI/UX 규칙 데이터베이스(`ui-ux-pro-max` 스킬)를 붙여 홈을 훑고, **재서 나온 것만** 손봤습니다. ① 🔴 **색은 «결정»으로 남겼습니다.** 브랜드 청록 `#00a79d` 가 어느 방향으로도 **2.99:1** 이었고(흰 배경 위 글자로도, 그 색 위에 흰 글씨를 얹어도) 그 값이 **버튼·링크·에러**에 전부 쓰이고 있었습니다. 통과값(`#007e77`/`#c64500`)으로 바꿔 **양쪽 화면을 만들어 비교**했는데, 브랜드 톤이 눈에 띄게 탁해져 **대표님이 원래 색을 선택**했습니다. 되돌렸고, 통과값과 그 값을 구한 방법은 F12 표·§7 «결정» 줄에 남겼습니다 — **다시 제안하기 전에 그 줄을 읽으세요.** ② 🔴 **홈에 간편 견적 폼이 두 벌 들어가 `id` 가 통째로 겹쳐 있었습니다** — 데스크톱은 히어로 안, 모바일은 히어로 아래이고 CSS 로 한쪽씩 숨기는데 **숨겨도 DOM 에는 남습니다.** 라벨이 화면에 없는 칸을 가리키고 있었고 오류는 안 났습니다(운영에서 중복 5건 실측 → 0건). `idPrefix` 로 갈랐습니다 → F5. ③ **슬라이드 인디케이터 버튼이 4px** 이라 손가락으로 못 눌렀습니다 — 막대는 그대로 두고 투명 여백으로 24px 을 만들었습니다. **자동 넘김 «정지» 버튼**을 넣고(터치 화면에는 hover 가 없습니다), 자동으로 넘어가는 동안에는 `aria-live` 를 껐습니다(6초마다 낭독이 끊깁니다). 짝 없는 `role="tab"` 도 걷어냈습니다 → F1·F12. ④ **입력칸 라벨을 눈에 보이게** 돌렸습니다 — 안내문만 있으면 타이핑을 시작한 순간 사라집니다. **에러 테두리가 청록**이던 것도 고쳤습니다(팔레트가 아니라 쓰던 자리가 틀린 경우라, 색을 되돌린 뒤에도 유지). ⑤ **아이폰 안전영역** — `viewportFit: "cover"` + `env(safe-area-inset-*)`. **둘은 항상 같이 갑니다**(하나만 있으면 CSS 는 그대로인데 효과가 0). ⚠️ **모바일 실기기 미확인**이고, **히어로 원본 사진이 848px 이라 2.9배 늘어나는 문제는 대체 사진이 없어 못 고쳤습니다** → §7) · 2026-08-10 (**`/works` 에 캐러셀형 구조화 데이터(ItemList) 추가 — F11.**
네이버 파워링크의 신규 확장소재 **'웹사이트정보'**(2025-05-29 출시, 광고그룹당 1개, 광고주 동의 필수)는
광고 문구를 직접 쓰는 게 아니라 **랜딩페이지의 구조화 데이터에서 캐러셀(ListItem)을 자동으로 긁어간다** —
susanna 마케팅 원장(`DeGaJa_Agent/users/hanjeonghyun/domains/susanna/notes.md` 2026-08-10)에서
확장소재 0개가 경쟁사 대비 화면 면적 열세로 확인됐고, 이게 그 대응입니다. 지금까지 이 사이트의
구조화 데이터는 `LocalBusiness`·`WebSite`·`BreadcrumbList`·`FAQPage`뿐이라 **캐러셀형이 하나도
없었습니다.** `/works` 가 이미 실사진·제목·분류를 가진 CMS 데이터를 그리고 있어 새 사실을 지어내지
않고 바로 재료로 썼습니다(`JsonLd.tsx` 의 "화면에 실제로 보이는 내용과 같아야" 규칙). 플레이스홀더
카드(사진 없는 실적)는 뺐습니다 — 없는 사진을 있다고 낼 수 없어서입니다. 개별 상세 URL이 없어(B-1
미착수, [SEO.md](SEO.md)) 전 항목이 `/works` 페이지 하나를 가리킵니다 — 상세 페이지가 생기면
각 항목의 `url` 을 그 페이지로 바꿔야 합니다. 🔴 **네이버가 요구하는 정확한 마크업 스펙(정확한
`@type`)은 확인하지 못했습니다** — `searchadvisor.naver.com` 가이드 페이지에 접근하지 못해
schema.org 표준 `ItemList`/`Thing` 으로 갔습니다(Google 캐러셀과 같은 어휘). **광고그룹에서
`웹사이트정보` 확장소재를 켜고 동의하는 건 사람이 합니다**(P5, ads.naver.com 계정 `2570727`) —
코드는 재료만 만들고, 켜는 건 여기 몫이 아닙니다. 켠 뒤 실제로 노출되는지는 나중에 실측해야
합니다.) · 2026-08-07 (**`/about` 의 FABRICATION 구역도 차단 · 스위치를 하나로 통합.** 홈만 껐던 탓에 같은 "사진 준비 중" 다섯 칸이 `/about` 에 그대로 나가고 있었습니다(대표님이 보고 요청). `app/page.tsx` 안의 지역 상수 `SHOW_HOME_FABRICATION` 을 **`config/content.ts` 의 `SHOW_FABRICATION`** 으로 옮겨 두 페이지가 같이 씁니다 — 둘로 두면 사진이 들어왔을 때 한쪽만 켜고 끝납니다. 딸려 나온 것 둘: ① 푸터의 **"공장 공정"(`/about#equipment`) 링크 제거** — 앵커가 사라져 누르면 `/about` 맨 위로만 떨어졌습니다 ② **관리자 화면 안내문 교정** — `config/sections.ts` 의 "제작 공정" 탭 `where` 가 "홈페이지 검은 배경의 FABRICATION 구역" 이라고 알려주고 있었는데 이제 어디에도 안 나갑니다. `README.md` 운영자 표에도 "지금은 아무 데도 안 나옴" 을 적었습니다 — **탭이 보이는데 반영이 안 되는 상태는 안 적어두면 고장으로 읽힙니다** → §2 트리 경고표) · 2026-08-07 (**`/about` 사진 통일 · 연차 13→14 교정 · `/process` 사진 칸 제거.** ① `/about` 인사말 사진을 홈과 **같은 제작+시공 합성 사진**으로 갈았습니다(대표님 요청) — 08-07 오전에 갈라 놓았던 두 자리가 다시 같은 사진이 됐고, **파일은 일부러 둘로 유지**합니다(한쪽만 갈아 끼울 수 있게, A6). `alt`·`label` 도 "사옥" 에서 실제 장면으로 고쳤습니다. ② **"13년째" → "14년째"** (`app/about/page.tsx` 제목·본문 2곳). 2012년 설립이라 2026년 기준 14년이 맞습니다. `site.founded` 에서 자동 계산하지 않는 이유를 주석으로 남겼습니다 — 세는 법이 사람마다 달라 대표님이 쓰시는 숫자와 어긋날 수 있습니다. **해 바뀌면 그 두 곳을 손으로 올려야 합니다.** ③ **`/process` 의 단계 사진 5칸 제거** — 실사진 0장이라 "사진 준비 중" 회색 상자만 나가던 자리로, 홈 PROCESS 를 같은 이유로 먼저 정리했고 혼자 남아 있던 이 페이지를 맞췄습니다. 사진이 빠져 글이 `max-w-6xl` 을 다 쓰게 되므로 본문은 `max-w-3xl` 로 묶었습니다. **이제 `step-*.jpg` 를 쓰는 화면이 없습니다** → §2 트리 경고표·§7 사진 부채·`public/images/README.md` 갱신) · 2026-08-07 (**홈 WHY SUSANNA 사진 재교체 — 공장 내부 장면 → 제작+시공 대각 합성 + 2x 업스케일.** 대표님이 주신 합성 사진(왼쪽 크레인 고소작업 시공 / 오른쪽 공장 제작 중인 채널문자)이 구역 문구 "제작과 시공이 나뉘면 서로를 가리킵니다" 를 정확히 받습니다. 원본 **1580×1180 이 육안으로 물러** `bigjpg.com` 무료(사진 모드·2x·노이즈 중간)로 **3160×2360** 을 받아 4:3 가운데 자르고 **1600×1200 q88 226KB** 로 넣었습니다 — **이 파일만 1200×900 규격을 벗어납니다**(모바일 `sizes="100vw"` 라 고밀도 화면에서 1200 이 모자람). `alt` 도 실제 장면으로 고쳤습니다. 업스케일 원본은 저장소에 없습니다 → `public/images/README.md`) · 2026-08-07 (**홈 WHY SUSANNA 사진 교체 — 사옥 외관 → 공장 내부 작업 장면.** 대표님이 주신 세로 사진(1086×1448)을 가운데 기준으로 잘라 1200×900 으로 넣었습니다(자리가 `aspect-4/3` 가로라 `object-cover` 가 어차피 자릅니다 — 미리 잘라 파일 크기를 줄이고 프레이밍을 확정). 사옥 외관 1장을 홈·`/about` 두 자리에 돌려쓰던 상태가 풀렸고, `alt` 도 실제 장면으로 고쳤습니다. 원본은 저장소에 없습니다 → `public/images/README.md`) · 2026-08-07 (**홈에서 빈 사진 자리 정리 — PROCESS 사진 칸 제거 · FABRICATION 구역 차단.** 둘 다 실사진이 없어 검은/회색 "사진 준비 중" 상자만 나가던 자리입니다(PROCESS 5칸 0장 · FABRICATION 6칸 중 1장). PROCESS 는 `<Img>` 블록만 걷어내 **글 상자만** 남겼고, FABRICATION 은 **지우지 않고** `app/page.tsx` 의 `SHOW_HOME_FABRICATION` 스위치로 껐습니다 — 공장 공정 사진이 들어오면 한 줄로 되살립니다. **DB·관리자 화면·`/process`·`/about#equipment` 는 그대로**라 관리자가 고쳐도 홈만 안 바뀝니다 → §2 트리의 경고 표 참조 · §7 사진 부채 갱신) · 2026-08-07 (**숫자 지표 띠(`stat` 구역) 제거 — 홈·`/about` 양쪽.** 4칸 격자에 1건만 남아 **빈 회색 칸 3개**가 보이던 상태였고, 관리자 화면에서 마지막 1건을 지워도 **0건 → config 폴백**으로 되살아나 없앨 수가 없었습니다 [A1의 부작용]. 코드에서 걷어냈습니다 — 두 페이지의 `<dl>` · `config/sections.ts` 명세(관리자 탭) · `lib/cms.ts` `SiteBlocks.stats` · `config/content.ts` `stats`/`Stat`. 마이그레이션 `0005` 와 DB 행은 그대로 둡니다 · §3 F19 에 "구역을 비울 수 없다" 함정 기록) · 2026-08-07 (**주요실적 4건 추가 — CMS `works batch`, 코드·배포 없음.** `belkin · NEWNESS 매장 사인` · `라온카페` · `영림중문 도어 매장 실내 사인` · `청춘예찬` — 전부 상업시설·대전, 사진은 Supabase 스토리지. **F18 CLI 의 실계정 DB 작업이 이때 처음 실증됐습니다**(로그인·`list`·`batch` 4건) · §7.2 미검증 항목 축소) · 2026-08-06 (**Google Analytics 4 설치 — F22 신설**(`G-NNT57E6S9L`, 확정 도메인에서만 로드) · **개인정보처리방침 위탁·쿠키 조항 신설**(법적 의무) · §7 에 GA 속성 3개 정리 + Ads 미완성 캠페인 부채 신설) · 2026-08-06 (**우편번호 교정 `35374`(추정) → `35425`(대표님 확인)** — 푸터·회사개요·오시는길과 **구조화 데이터 `postalCode`** 5곳에 틀린 값이 나가고 있었습니다 · **구글 Search Console 색인 요청 5건**(`/works` `/about` `/process` `/support` `/quote`) · **구글 비즈니스 프로필 소유권 주장 진행중** — 기존 미소유 등록의 카테고리·주소·전화를 교정 · §7 갱신) · 2026-08-06 (**네이버 예약 개설 — 검수 신청까지 완료.** 예약 사업자 `1713161` · 상품 `간판 무료 상담`(날짜선택형·무료·노출중) · 상세소개에서 **사진/도면은 `susannadesign.co.kr/quote` 로 유도** · 적는 칸은 자유 입력(기타 요청사항) · §7 부채 1건 신설 · [`SEO.md`](SEO.md) C1-b 신설) · 2026-08-06 (**오시는길 링크 카드 — F21 신설.** `map.naver.com` 이 `x-frame-options: DENY` 라 지도 임베드가 불가능해, 주소·영업시간 + 네이버 플레이스·카카오맵 길찾기·전화 버튼으로 대체. `/support` 는 정적 유지) · 2026-08-06 (**네이버 스마트플레이스 — 가격정보 9건·영업시간·부가정보 직접 입력 완료**(플레이스 ID `1378209445`) · **`site.naverPlaceUrl` 신설 → `sameAs` 연결** · **`site.geo` 를 등록 좌표로 교체 — 이전 근사값이 실제 위치에서 약 10km 벗어나 있었음** · `site.mapUrl` 채움 · §7 부채 2건 해소·1건 신설(대표키워드·전화번호는 "업체 검토중" 으로 잠김)) · 2026-08-06 (**공장 전경 사진 투입 — 홈 `about-factory.jpg` + `/about` `about-office.jpg` · §7 사진 부채에서 "사옥 전경 1" 해소** · `public/images/README.md` 에 홈 자리가 누락돼 있던 것을 F4 함정으로 기록 · 알림 환경변수 `Secret` 재등록 완료) · 2026-08-06 (**견적 첨부파일 실보관 — F20 · `quote-files` 비공개 버킷 신설 · 알림 메일에 사진 직접 첨부 · `0006` 운영 실행 완료** · **Resend `susannadesign.co.kr` 도메인 인증 완료** — 회사 주소로 발송·수신자 2인) · 2026-08-06 (**페이지 문구 CMS — F19 · P14 `/admin/content` · `content_blocks` 표 신설** · **견적 알림 메일 미도착 원인 규명·수정** — 발신 도메인 미인증 403, 시험 발신 주소로 자동 우회) · 2026-07-28 (**당근 비즈프로필 등록 — `수산나디자인 간판제작`** · 채널 전략 4종(당근광고·숨고·플레이스→홈페이지 유입·블로그) [`SEO.md`](SEO.md) C1·C5·C6·C8 · **히어로 인사 슬라이드 추가 → 4장** · **네이버 소유확인 완료** · **URL 검사 느낌표 5건 해소** · 실적 사진 12장 투입 · F18 CMS 명령줄 도구 · IndexNow F17) |
...
| 사진 | ⚠️ 실사진 **21장**(실적 17 · 사업영역 4 · 공정 1 · **공장/사옥 2**, 일부 공용). 2026-08-07 홈 `about-factory.jpg` 와 `/about` `about-office.jpg` 를 **같은 제작+시공 대각 합성 사진**으로 갈았습니다 — 이 둘만 **1600×1200**(다른 자리는 1200×900). **두 파일 내용이 지금 동일합니다**(대표님 요청). 파일은 일부러 둘로 유지 — 한쪽만 갈아 끼울 수 있게(A6). 남은 자리는 §7 |
...
| **A3** | 공개 페이지에서 **쿠키를 읽지 않는다** — **예외 하나**(아래) | 쿠키를 읽는 순간 정적 생성이 깨져 요청마다 SSR이 됨 |
...
> ### 🔴 A3 의 유일한 예외 — 새 디자인 미리보기 (2026-08-17, F23)
>
> **대표님이 근거를 듣고 연 문입니다.** 새 디자인을 **손님이 실제로 보는 그 주소에서**
> 검증하려면 공개 화면이 방문자를 알아봐야 하고, 그건 쿠키 말고는 방법이 없습니다.
>
> **읽는 곳은 `lib/preview.ts` 한 곳뿐이고, 부르는 곳은 `app/layout.tsx` 한 줄뿐입니다.**
> 다른 데서 공개 페이지가 `cookies()` 를 부르기 시작하면 이 예외는 규칙이 되고, 그때는
> 되돌릴 자리를 못 찾습니다.
>
> **A3 가 원래 지키려던 것과, 그중 무엇을 내줬나**
>
> | A3 의 근거 | 지금 상태 |
> |---|---|
> | "정적 생성이 깨진다" | **이미 깨져 있었습니다** — DB 를 읽는 다섯 페이지가 `force-dynamic`(§2.1). 이번에 **나머지 다섯 장(`/support` `/quote` `/privacy` `/terms` `/no-email-collect`)도 동적**이 됐습니다. 빌드 결과에서 `○` 가 사라진 것으로 확인했습니다 |
> | "나중에 CDN 캐시를 얹을 길이 막힌다"(F7) | **여기가 진짜 대가입니다.** §7 에 부채로 적었습니다 |
>
> 🔴 **캐시를 되살릴 때 반드시 같이 할 일** — Cache Rules 를 켜면서 **`susanna-preview`
> 쿠키가 있는 요청을 우회(bypass)**시키지 않으면, **대표님이 보던 새 디자인이 캐시에
> 담겨 손님에게 나갑니다.** 지금은 공개 HTML 이 `no-store` 라 안 나지만, §7 의 SSR
> 부채를 갚는 순간 터지는 종류입니다.
>
> **권한은 쿠키가 아니라 `getCurrentUser()` 가 판단합니다** [A2]. 쿠키는 "보고 싶다"는
> 뜻일 뿐이고 `httpOnly` 라 서버 액션(`setPreview`)만 심을 수 있습니다.
...
| **P15** | `/products` | `app/products/page.tsx` | **요청 시 SSR** (F24) | **CMS** `getBlocks().products` |
...
| **P7** | `/quote` | `app/quote/page.tsx` | **요청 시 SSR** (F23) | `content.ts` 선택지 |
...
> **2026-08-17 — 정적이던 다섯 장이 동적이 됐습니다** (F23). 루트 레이아웃이 쿠키를
> 읽으면 그 아래 전부가 동적이 됩니다. 남은 `○`(정적)는 `robots.txt` `sitemap.xml`
> `indexnow.txt` 셋뿐입니다 — 라우트 핸들러라 레이아웃을 안 거칩니다.
> **되돌리는 법**: `app/layout.tsx` 의 `getPreview()` 한 줄을 빼면 그대로 돌아옵니다.
...
├── PROCESS 5단계        ← CMS: content_blocks(process) — **글 상자만. 사진 칸 없음**
├── 주요실적 6건         ← CMS: works (앞에서 6개)
├── FABRICATION 6종      ← CMS: content_blocks(fabrication) — **지금 꺼져 있음**
│                           `SHOW_FABRICATION = false` (config/content.ts)
│                           — `/about` 의 같은 구역과 스위치를 공유합니다
└── CTA 밴드            ← config/site.ts
```

> **홈에서 사진이 빠진 두 자리 (2026-08-07)** — 둘 다 이유가 같습니다. 실사진이
> 없어서 "사진 준비 중" 회색 상자만 줄지어 나가고 있었습니다.
>
> | 자리 | 어떻게 | 되살리는 법 |
> |---|---|---|
> | PROCESS 5칸의 사진 | `<Img>` 블록 **삭제** | git 이력에서 그 블록을 가져다 붙입니다 |
> | FABRICATION 구역 전체 (**홈 + `/about`**) | `config/content.ts` 의 `SHOW_FABRICATION` **스위치로 차단** | 그 값을 `true` 로 + 푸터의 "공장 공정" 링크 복구 |
>
> **2026-08-07 (2차) — `/process` 의 사진 칸도 같은 이유로 걷어냈습니다.** 위 표의
> "되살리는 법"이 원래 "`app/process/page.tsx` 의 같은 블록을 가져다 붙입니다" 였는데,
> 그 블록이 사라졌으므로 **git 이력으로 바꿨습니다.** 사진이 빠지면서 글이 `max-w-6xl`
> 을 다 쓰게 돼 한 줄이 너무 길어지므로 본문은 `max-w-3xl` 로 묶었습니다.
> **이제 `step-*.jpg` 를 쓰는 화면이 하나도 없습니다** — `public/images/` 에 넣어도
> 아무 데도 안 나옵니다(`public/images/README.md` 에 경고를 적어 뒀습니다).
>
> **2026-08-07 (3차) — `/about` 의 FABRICATION 구역도 내렸고, 스위치를 하나로 합쳤습니다.**
> 홈만 껐던 탓에 같은 "사진 준비 중" 다섯 칸이 `/about` 에 그대로 나가고 있었습니다.
> 스위치가 `app/page.tsx` 안의 지역 상수(`SHOW_HOME_FABRICATION`)였는데,
> **`config/content.ts` 의 `SHOW_FABRICATION` 하나로 옮겼습니다** — 둘로 두면 사진이
> 들어왔을 때 한쪽만 켜고 끝납니다. 딸려 나온 것 하나: 푸터의 **"공장 공정"
> (`/about#equipment`) 링크를 뺐습니다.** 앵커가 사라져 누르면 `/about` 맨 위로만
> 떨어졌습니다 — 스위치를 켤 때 그 줄도 같이 살려야 합니다(`components/Footer.tsx`).
>
> ⚠️ **DB·관리자 화면은 그대로입니다.** `/admin/content` 의 "업무 프로세스 → 사진"
> 칸과 "제작 공정" 탭이 계속 보이는데, **둘 다 이제 어느 화면에도 안 나갑니다**
> (업무 프로세스의 *글 문구*는 홈·`/process` 양쪽에 정상 반영됩니다 — 안 나가는 건
> 사진 칸뿐입니다). F19 의 "구역을 비울 수 없다" 함정과 같은 계열이니 헷갈리지 마세요.
...
components/QuickQuoteForm.tsx  히어로 인라인 3필드   ← idPrefix 필수 (아래 함정)
...
- 🔴 **홈에는 `QuickQuoteForm` 이 두 벌 들어갑니다 — `idPrefix` 를 반드시 다르게 주세요**
  (2026-08-14 수정). 데스크톱은 히어로 사진 안(`HeroSlider`), 모바일은 히어로 아래
  (`app/page.tsx`)이고 CSS 로 한쪽씩 숨깁니다. **숨겨도 DOM 에는 둘 다 남습니다.**
  그전까지 `q-name`·`q-phone`·`q-region`·`q-website`·`agree` 가 **전부 두 번씩** 있었고,
  브라우저는 `label for=` 를 문서에서 **처음 만난 칸**에 붙입니다 — 즉 모바일에서 라벨을
  눌러도 화면에 없는 데스크톱 칸에 포커스가 갔고, `aria-describedby` 의 에러 안내도
  같은 곳을 가리켰습니다. **오류가 안 나서 화면만 봐서는 모릅니다.**
  운영 페이지에서 `id` 목록을 뽑아 중복 5건을 확인했고, 고친 뒤 0건입니다.
  `PrivacyConsent` 도 같은 이유로 `idPrefix` 를 받습니다.
- ⚠️ **입력칸 라벨을 `sr-only` 로 돌리지 마세요** (2026-08-14 수정). 간편 폼이 그랬는데,
  안내문(placeholder)은 **타이핑을 시작하면 사라집니다** — 세 칸을 채우고 나면 무엇을
  적는 칸이었는지 화면에 단서가 없고, 자동완성으로 채워지면 처음부터 없습니다.
- ⚠️ **에러 상태 테두리가 `border-brand`(청록)였습니다** — 잘못된 값에 브랜드색이
  들어와 "정상"처럼 보였습니다. 지금은 `accent-700`(F12 색 표) 입니다.
...
app/works/page.tsx → ItemList JSON-LD (2026-08-10)  ★ 네이버 파워링크 '웹사이트정보' 확장소재용
├── 실사진 있는 실적만(`imageExists`) — 플레이스홀더는 "화면에 없는 걸 낸다"가 되어 제외
├── 각 item 은 `@type: Thing`(name·image·url) — 개별 상세 URL이 없어 전부 `/works` 를 가리킴
│   B-1(시공사례 개별 페이지, SEO.md)이 생기면 항목별 url 을 그 페이지로 바꿀 것
└── ⚠️ 네이버의 정확한 캐러셀 마크업 스펙은 미확인 — schema.org 표준(Google 캐러셀과 동일 어휘)으로
    감. 광고그룹에서 확장소재를 켜는 건 사람이 함(P5) — 켠 뒤 실제 노출 여부 실측 필요
...
#### 색 명암비 — **재봤고, 안 고치기로 했습니다** (2026-08-14)

🔴 **이 사이트의 색은 WCAG AA(본문 4.5:1)를 못 넘깁니다. 모르고 그런 게 아닙니다.**
통과값까지 만들어 화면을 띄워 놓고 비교한 뒤 **대표님이 원래 색을 선택**했습니다 —
회사 색이 회사소개서에서 온 값이라 접근성보다 우선한다는 판단입니다.
**"덜 고친 것"으로 오해하고 다시 손대지 마세요.** 되살리려면 아래 값을 쓰면 됩니다.

| 토큰 | 지금 값 | 흰 배경 | paper | ink 배경 | 통과값(계산해 둔 것) |
|---|---|---:|---:|---:|---|
| `brand` | `#00a79d` | 2.99 ❌ | 2.88 ❌ | — | `#007e77` (4.94 / 4.52) |
| `accent` | `#ff5900` | 3.14 ❌ | 2.88 ❌ | 5.65 ✅ | 밝은 배경용 `#c64500` 별도 토큰 |
| placeholder | `ink-500/60` | 2.50 ❌ | — | — | 불투명 `ink-500` (5.61) |
| `brand-400` | `#35c2b8` | 1.9 ❌ | — | 8.09 ✅ | **어두운 배경 전용** — 밝은 배경에 쓰지 마세요 |

- **통과값을 구한 방법**: 색을 HSV 로 바꿔 **색상(hue)·채도는 고정하고 명도만** 낮추면서
  처음으로 4.5:1 을 넘는 값을 찾습니다. 브랜드에서 최소한으로만 멀어집니다.
- ⚠️ **흰 배경만 보고 정하면 틀립니다.** 홈 PROCESS 구역이 `bg-paper`(#f5f5f2) 라
  기준이 더 빡빡합니다 — `#00857d` 는 흰 배경(4.52)은 통과하는데 paper(4.13)에서 깨집니다.
  실제로 쓰는 배경 세 개(흰 · paper · brand-50)를 **전부 넣고** 풀어야 합니다.
- ⚠️ **`accent` 는 한 값으로 양쪽을 못 씁니다.** 밝은 배경과 어두운 배경의 요구가
  정반대입니다(3.14 ❌ / 5.65 ✅). 고치게 되면 토큰을 둘로 갈라야 합니다.
- ⚠️ **컴포넌트에 색을 손으로 박지 마세요.** `PrivacyConsent` 의 체크박스가
  `accent-[#00a79d]` 로 박혀 있어서 **팔레트를 고쳐도 거기만 옛 색으로 남는** 자리였습니다.
  지금은 `accent-brand` 토큰을 씁니다(값은 동일). 이건 색 결정과 무관하게 유지합니다.
- **에러 테두리는 예외로 고쳤습니다.** `QuickQuoteForm` 의 입력칸이 `aria-invalid` 일 때
  `border-brand`(청록)로 변했습니다 — 잘못된 값에 브랜드색이 들어와 «정상»처럼 보였습니다.
  팔레트를 바꾼 게 아니라 **쓰던 자리가 틀렸던 것**이라 `accent`(에러색)로 두었습니다.

#### 히어로 슬라이더 (F1) 접근성 (2026-08-14)

- **자동 넘김 «정지» 버튼**을 넣었습니다. hover·focus 로 멈추는 장치는 원래
  있었지만 그건 **손을 대고 있는 동안만**이고, 터치 화면에는 hover 가 없습니다.
  «사람이 누른 정지»(`stopped`)와 «마우스가 올라간 상태»(`hovering`)를
  **일부러 다른 상태로 둡니다** — 하나로 두면 정지를 누른 뒤 마우스가 나가는 순간
  다시 돌아가서 버튼이 있으나 마나가 됩니다.
- **`aria-live` 는 자동 넘김 중에 꺼집니다.** 6초마다 읽어 주면 낭독이 계속 끊깁니다.
  사람이 버튼으로 넘겼을 때만 켭니다.
- **인디케이터 막대는 4px 이지만 버튼은 24px** 입니다(실측 32×24). 예전에는 버튼
  자체가 4px 라 손가락으로 못 눌렀습니다. 막대를 두껍게 하지 않고 **투명 여백**으로
  크기를 만듭니다 — WCAG 2.2 «Target Size (Minimum)» 24×24.
- **`role="tab"` 을 걷어냈습니다.** 탭 역할은 `tabpanel` 과 짝이어야 하는데 슬라이드는
  패널이 아니라, 스크린리더에 "탭 4개"라고 잘못 알려 주고 있었습니다.
- `prefers-reduced-motion` 은 **state 로 읽습니다**(ref 아님). 정지 버튼을 보일지가
  그 값에 걸려 있어서, ref 로 두면 값이 바뀌어도 화면이 다시 안 그려집니다.

#### 안전영역 (아이폰 노치·홈 인디케이터) (2026-08-14)

```
app/layout.tsx   viewport.viewportFit = "cover"     ← 이게 없으면 아래 둘이 전부 0
app/globals.css  body padding-bottom  calc(4.5rem + env(safe-area-inset-bottom))
                 .wrap padding-inline max(고정값, env(safe-area-inset-left/right))
components/FloatingBar.tsx  pb-[env(safe-area-inset-bottom,0px)]
```
- 🔴 **`viewportFit: "cover"` 와 `env()` 는 항상 같이 갑니다.** 하나만 있으면
  **CSS 는 그대로인데 아무 효과가 없습니다**(env() 가 0 을 돌려줌). 고정바가
  홈 인디케이터에 물려 버튼 아래쪽이 안 눌리던 문제가 그것입니다.
- `cover` 를 켜면 가로모드에서 콘텐츠가 노치 밑으로 들어갈 수 있어 `.wrap` 의 좌우
  여백을 `max()` 로 받습니다. 세로모드·PC 에서는 env() 가 0 이라 예전과 같습니다.
- ⚠️ **모바일 실기기에서는 아직 확인하지 않았습니다** (§7.2).
...
### F24. 제품 카탈로그 (`/products`) — 🔴 아직 안 연 페이지
```
config/content.ts
├── products[]        DB 가 죽었을 때의 폴백 [A1]
└── SHOW_PRODUCTS     🔴 손님에게 열지 말지. 지금 false — 켜는 건 대표님 결정

supabase/migrations/0008_product.sql
└── content_blocks 의 section 목록에 'product' 추가 + 초기 6건
    eyebrow=분류(필터 탭) · title=제품명 · sub=한 줄 설명 · points=유형 키워드

보이는 조건 (세 곳이 «한 값»에서 갈립니다)
├── app/products/page.tsx   !SHOW_PRODUCTS && !preview.on  → notFound()
├── components/Header.tsx   productsVisible → 주 메뉴에 "제품"
└── config/site.ts          noindexPaths 에 자동 포함 → app/sitemap.ts 가 자동 제외
```
- **왜 이렇게 생겼나** — 쿠팡·네이버쇼핑·오늘의집·아트네온(대전 경쟁사) 네 곳의 제품
  리스트를 실제 크롬으로 열어 DOM 을 재고 정했습니다. 근거 전문은
  [`reference/reference.md`](../reference/reference.md) **부록 A**. 요약:
  - 🔴 **썸네일은 정사각(1:1).** 네 곳 전부 그랬고 예외가 없었습니다.
    **실적(`aspect-4/3`)과 일부러 다릅니다** — 합치면 제품 목록이 실적 목록처럼 보입니다.
  - 🔴 **가격 칸이 없습니다.** 아트네온은 Cafe24 쇼핑몰인데도 목록에 가격을 안 띄웁니다.
    맞춤 제작이라 정가가 성립하지 않아서고, 우리도 같습니다.
    가격을 넣자는 얘기가 나오면 §7 의 «가격 예시» 줄과 부록 A 를 먼저 읽으세요.
  - **분류 탭은 코드에 고정하지 않고 `eyebrow` 값에서 모읍니다.** 고정하면 관리자가 새
    분류를 적었을 때 그 제품이 어느 탭에도 안 걸립니다.
  - ⚠️ **별점·리뷰수·판매량 칸을 흉내 내지 마세요.** 네이버가 그걸 다 넣는 건 숫자가
    **있기 때문**입니다. 우리는 없어서, 만들면 빈 칸이 줄지어 나갑니다 — FABRICATION 과 같은 실패.
- 🔴 **`SHOW_PRODUCTS` 가 꺼져 있으면 손님에게는 404 입니다.** "링크만 안 걸면 된다" 로
  두지 않았습니다 — 주소는 짧고 추측 가능하며, `noindex` 는 «색인하지 말라»지
  «보여주지 말라»가 아닙니다. 관리자는 **F23 미리보기**로 봅니다.
- ⚠️ **메뉴와 페이지가 한 값에서 갈리는 게 설계입니다.** 페이지는 404 인데 주 메뉴에만
  "제품" 이 서 있는 상태가 제일 나쁩니다.
- ⚠️ **초기 사진 6장은 실적 사진을 다시 씁니다** (제품 전용 촬영본이 없습니다).
  사진은 한 장씩 열어 보고 골랐고, 그 과정에서 둘을 교체했습니다 — `work-23`(홈센터)은
  계산대 잡동사니와 날짜 스탬프가 찍혀 있었고, `work-19`(자자고호텔)는 후광 채널이라
  «채널 간판» 과 겹쳤습니다. **제품 이름도 사진에 맞춰 고쳤습니다**
  ("층별 안내·유도 사인" → "실내 사인·스카시 문자") — 보여줄 수 없는 것을 이름으로
  약속하지 않으려는 것입니다 [P6]. → §7 부채
- **상세 페이지는 없습니다.** 카드 전체를 링크로 만들지 않고 «이 제품 견적 받기»(→`/quote`)
  로 보냅니다. 없는 상세 페이지를 가리키는 링크보다 낫고, 실제로 일어나는 일이기도 합니다.

#### F24-a. 재질 — `/products` 안의 평면 목록 (2026-09-05 신설)
```
config/content.ts
├── materials[]        지금은 유일한 원본. DB 연결 없음(F19 의 product 섹션과 다름)
└── Material 타입       key(파일명 대응) · name · desc · image · alt

components/MaterialsGrid.tsx
└── ProductsGrid 와 같은 정사각(1:1)·가격 없음 규칙. 분류 탭은 없음(서버 컴포넌트)

app/products/page.tsx
└── ProductsGrid 바로 아래, 하단 CTA 위에 위치. SHOW_PRODUCTS 게이트를 그대로 씁니다
    (별도 스위치를 만들면 켜고 끄는 값이 두 개가 되어 상태 추적이 어려워집니다)
```
- **왜 평면인가** — 재질끼리 묶을 분류축(계열? 색?)이 아직 안 정해졌습니다. 있는 걸
  그대로 다 보여주는 것으로 시작하고, 제품(간판 유형)과 묶어 "이 재질 위의 이 간판"
  조합을 보여주는 건 다음 단계입니다.
- **사진은 3D 렌더 10장**입니다(화강석·벽돌·콘크리트·파사드 통짜·대리석·금속·녹슨 금속·
  도장·타일·목재). 형제 저장소 `DeGaJa_Agent` 의 `users/hanjeonghyun/domains/susanna`
  `blender/_lab/03_signtypes/_catalog`(재질 58종 × 간판 9종 = 522장 렌더 카탈로그) 중
  대표 10장을 골라 `public/images/material-<key>.jpg` 로 복사했습니다. **실사진이 아니라는
  걸 화면 문구로 밝힙니다** — 렌더를 실제 시공 사진처럼 보이게 두지 않습니다 [P6].
- ⚠️ **DB 연결이 없습니다.** `product` 섹션과 달리 `content_blocks` 에 대응하는 `section`이
  없어 관리자 화면에서 못 고칩니다 — 지금은 `config/content.ts` 코드 수정 + 배포가
  유일한 경로입니다(`AGENTS.md` "사업영역·공정·장비·회사정보" 줄과 같은 취급).
- 🔴 **`SHOW_PRODUCTS` 를 임시로 `true` 로 바꿔 로컬에서 화면을 확인한 뒤 `false` 로
  되돌렸습니다.** 이 값은 대표님 결정이라 커밋에 `true` 로 남기지 않습니다.

#### F24-b. 카드·탭 배치 개편 (2026-09-08)
```
app/products/page.tsx
├── 가운데 정렬 큰 제목 → 알약 탭 줄 → 격자 → 맨 아래 가로 띠
└── CatalogTabs 에 panels={{signs, materials}} 로 서버에서 그린 격자를 넘깁니다

components/CatalogTabs.tsx   (신설, 클라이언트)
└── 「전체 · 간판 종류 · 재질」 — 목록 전환 탭. ProductsGrid 의 eyebrow 분류 탭과 다릅니다

components/SignTypesGrid.tsx (개편)
└── 230px 고정 타일 → 반응형 3열 카드(sm:2 · lg:3)
    사진(1:1) → 이름+가격 → 사양 칩(points[0] 을 「·」로 쪼갬) → 가로 꽉 찬 CTA

components/MaterialsGrid.tsx (개편)
└── 같은 테두리·모서리로 맞추되 크기는 그대로 작게. SignTypesGrid 와의 230×230 짝은 끊었습니다
```
- **왜 바꿨나** — 대표님이 렌터카 템플릿 화면을 주며 **"레이아웃만 저 느낌으로, 색이랑
  디자인은 수산나 그대로"** 라고 정했습니다. 가져온 것은 **배치 넷**뿐입니다 —
  ①가운데 큰 제목 ②알약 탭 줄 ③테두리 있는 3열 카드 ④맨 아래 가로 띠.
  **팔레트·타이포·정사각 썸네일은 하나도 안 바꿨습니다**(참고 화면의 보라 `#5B36F2` →
  브랜드 청록 `#00a79d`, 카드 바탕 `paper`, 띠 테두리 `line`).
- 🔴 **탭 축이 참고 화면과 다릅니다 — 축을 지어내지 않았습니다** [P6]. 저기는
  «세단·SUV» 같은 제품 속성인데, 간판 종류의 `eyebrow` 는 분류가 아니라 T1~T9
  **일련번호**라 그 자리에 넣을 축이 없습니다. 발광/무점등 같은 분류를 코드에서
  만들면 실측이 아니라 추측입니다. 대신 **이 페이지가 들고 있는 목록 두 개**로 세웠습니다.
  ⚠️ **재질 쪽에는 축이 있습니다**(`eyebrow` 대분류 — 2026-09-08 실측 DB 15종 = 6개
  대분류). 안 붙인 이유는 **15종이 한 화면에 다 들어와서**입니다. 재질이 늘면 그때
  `MaterialsGrid` 안에 붙이세요.
- 🔴 **사양 칩은 결국 안 냅니다 — 같은 날 두 번 뒤집혔습니다.** 새 카드가 «가격과
  사양이 각자 자리를 갖는» 배치라 2026-09-05 에 내렸던 칩을 잠깐 되살렸는데, 대표님이
  화면을 보고 **다시 빼라고** 했습니다(`60mm · 벽 이격 60mm · 빛이 벽으로 샘` 같은 줄이
  손님에게 쓸모없다는 판단). **세 번째로 되살리지 마세요** — 되살릴 거면 그 전에
  «이 문장이 손님의 무슨 결정을 돕나» 부터 답이 있어야 합니다. 값 자체는 `points[0]` 에
  그대로 살아 있습니다(`SIGNTYPES.md` 대조용).
  ⚠️ 칩을 빼면서 격자 아래 문구에서 **«카드에 적힌 치수는 모델 기준값» 조항도 같이
  뺐습니다**(화면에 치수가 없으니 가리킬 대상이 없습니다). 🔴 **렌더 고지는 남겼습니다**
  — 사진이 실사진이 아니라는 걸 밝히는 유일한 자리입니다 [P6].
  ⚠️ 칩을 빼자 **버튼이 이름 줄에 딱 붙었습니다** — `mt-auto` 는 남는 공간이 있을 때만
  밀기 때문입니다. 바깥 `div` 에 `pt-4` 를 줬습니다(버튼에 주면 버튼이 두꺼워집니다).
- ⚠️ **`SignTypesGrid` 와 `MaterialsGrid` 의 «230×230 짝»(2026-09-05 사람 지시)은
  끊었습니다.** 잊은 게 아니라 성격이 갈렸습니다 — 간판 종류는 가격·사양·CTA 를 든
  «파는 카드» 고, 재질은 이름표 한 줄짜리 «견본 조각» 입니다. 재질까지 큰 카드로
  키우면 가격·사양 칸이 통째로 비어 줄지어 나갑니다(FABRICATION 과 같은 실패).
- ⚠️ **아래 띠를 먹색으로 짰다가 되돌렸습니다** — 사이트 공통 푸터가 이미 먹색이라
  두 덩이가 붙어 서면 띠가 푸터의 일부로 읽힙니다. 참고 화면의 그 띠도 밝은 바탕입니다.
- 🔴 **`SHOW_PRODUCTS` 를 임시로 `true` 로 바꿔 로컬에서 화면을 확인한 뒤 `false` 로
  되돌렸습니다**(F24-a 와 같은 절차). 확인: `npm run lint`·`npm run build` 통과,
  헤드리스 크롬으로 **1440px·520px** 두 폭을 떠서 봤습니다(가격 없는 T8 은
  "가격 확인 필요" 로, 사양이 한 조각뿐인 T5·T6·T8 은 칩 한 개로 정상).
  ⚠️ **390px 실측은 못 했습니다** — 헤드리스 크롬의 최소 창 폭에 걸려 오른쪽이 잘립니다.
  520px 도 `sm`(640) 아래라 같은 단일 열 배치지만, **폰 실기기 확인은 아직입니다** [P6].
- 🔴 **화면을 뜰 때 창 높이를 페이지보다 길게 잡지 마세요 — 없는 여백이 생깁니다.**
  `body flex min-h-full` + `main flex-1` + `footer mt-auto` 라서 **창이 내용보다 길면
  그 차이만큼 푸터가 아래로 밀립니다.** 실제로 2026-09-08 에 `--window-size=1440,5200`
  으로 뜬 그림을 보고 «띠와 푸터가 벌어졌다» 는 지적이 나왔는데, 실제 브라우저에서 잰
  값은 **80px 로 `/works` 와 동일**했습니다(고칠 것 없음으로 종결). 전체 페이지가
  필요하면 창을 늘리지 말고 CDP 의 `captureBeyondViewport`
  (`python -m module_browser fast shot <경로> --full`)를 쓰세요.

### F23. 새 디자인 미리보기 (같은 주소, 관리자에게만) ★
```
lib/preview.ts (server-only)
└── getPreview()  React cache 로 렌더당 1회
    ├── sb-* 쿠키 없음        → 즉시 종료. 손님은 인증 왕복 0회   ← 이 빠른 길이 핵심
    ├── getCurrentUser()      → role !== "admin" 이면 꺼짐  [A2]
    └── { isAdmin, on, stale }
                       stale = 쿠키는 켜져 있는데 관리자 판정 실패 (= 로그인 만료)

app/layout.tsx  ← 🔴 A3 예외를 쓰는 **유일한** 자리 (§0)
├── <PreviewBar state={preview} />              관리자에게만. 서버 컴포넌트 = JS 0
└── (isAdmin || stale) && <SessionKeeper />     next/dynamic 으로 늦게 부름

app/admin/actions.ts  setPreview(formData)
├── requireAdmin()  먼저                         [A2]
├── 쿠키 susanna-preview = "1" (httpOnly · sameSite lax · 7일)
└── revalidatePath("/", "layout")
```
- **화면을 갈라 그리는 법** — 그 페이지에서 `getPreview()` 를 부르고 `on` 으로 갈립니다.
  `cache` 되어 있어 몇 번을 불러도 인증 왕복은 렌더당 한 번입니다.
- 🔴 **화면 문구에 "시안" 을 쓰지 마세요.** 이 회사는 그 말을 **손님에게 내는 간판 디자인
  시안**이라는 뜻으로 이미 쓰고 있습니다(홈 PROCESS "시안 제작"·"시안 확정"). 같은 화면에서
  두 뜻으로 쓰이면 대표님이 이 막대를 견적 기능으로 읽습니다. 그래서 **"새 디자인" · "손님이
  보는 화면"** 으로 적었습니다.
- 🔴 **`stale`(로그인 만료)을 조용히 `off` 로 삼키지 마세요.** 접속 토큰은 1시간이면
  만료되는데, 그때 그냥 원래 화면을 보여주면 대표님 눈에는 **"켰는데 안 바뀐다"** 로만
  보이고 원인이 화면 어디에도 없습니다. 쿠키가 `httpOnly` 라 관리자 인증을 통과한 서버
  액션 말고는 심을 수 없으므로, 이 상태는 사실상 "만료된 관리자" 하나뿐입니다.
- ⚠️ **`SessionKeeper` 를 그냥 `import` 하지 마세요.** 조건부로 그려도 `@supabase/ssr` 이
  **모든 공개 페이지의 공용 번들**에 들어갑니다. `next/dynamic` 이라야 관리자일 때만
  그 조각을 받아 옵니다. 이게 있어서 공개 페이지를 도는 동안 토큰이 갱신되고, 위 `stale`
  상태에 잘 안 빠집니다.
- ⚠️ **`PreviewBar` 에 `"use client"` 를 붙이지 마세요.** 지금은 `<form action={서버액션}>`
  뿐이라 클라이언트 JS 가 0 이고, 손님 HTML 에는 **마크업도 안 나갑니다**(실측: 익명 응답에
  `setPreview`·막대 클래스·`SessionKeeper` 전부 0건).
- ⚠️ **막대 자리 — 데스크톱에서 `bottom-6` 은 안 됩니다.** 개발 서버의 **Next 표시등**이
  왼쪽 아래에 앉아 "새 디자인 보는 중" 의 앞글자를 가립니다(2026-08-17 크롬 실측).
  배포본에는 그 표시등이 없지만 **새 디자인을 들여다보는 시간의 대부분이 개발 서버**라
  거기 맞춰 `md:bottom-20` 으로 띄웠습니다. 모바일 값(`4.25rem` + 안전영역)은 하단
  고정바(`FloatingBar`)를 피하는 높이라 따로입니다.
- **실측 (2026-08-17, 크롬 CDP)**: 관리자 로그인 후 홈에 막대 표시 → 눌러서 켜짐 →
  `/works` 로 이동해도 유지 → 다시 눌러 꺼짐. `document.cookie` 에 `susanna-preview` 가
  **안 보입니다**(`httpOnly` 확인). 익명 응답에는 막대 마크업·`setPreview`·`SessionKeeper` 0건.
- ⚠️ **"안 보인다" 는 신고가 오면 순서대로 보세요** — ① 그 주소에 **배포됐는가**(로컬만
  고쳐 놓고 운영을 보는 경우) ② **로그인이 살아 있는가**(`/admin` 이 로그인 화면으로
  튕기면 만료입니다. 쿠키는 브라우저에 남아 있어도 서버 검증에서 무효일 수 있습니다)
  ③ `profiles.role` 이 `admin` 인가. 2026-08-17 에 ①과 ②가 동시에 걸렸습니다.
- **대가**: 정적이던 다섯 페이지가 동적이 됐고(§2.1), CDN 캐시를 얹을 때 쿠키 우회 설정이
  필수가 됐습니다(§0 A3 예외 · §7).
...
```
app/support/page.tsx  (P6 — 2026-08-17 부터 요청 시 SSR, F23)
...
| `0008_product.sql` | **제품 구역** — `content_blocks` 의 `section` 목록에 `'product'` 추가 + 초기 6건 (F24). 🔴 **아직 안 돌렸습니다.** 안 돌려도 화면은 `config/content.ts` 폴백으로 정상이지만 [A1], **돌려야 관리자 화면 "제품" 탭에서 고칠 수 있습니다** |
...
| 구분 | 내용 | 위치 |
|---|---|---|
| 🔴 사진 | **남은 자리: 실적 4자리 + 히어로 2 + 공정 5.** 회사소개서에서 12장을 뽑아 넣어 실적 15자리 중 11자리·사업영역 4자리 전부가 채워졌고(2026-07-27), **사옥 전경은 대표님이 주신 현장 촬영본으로 채웠습니다**(2026-08-06). 급한 순서: ① **히어로 2·3 — 현장 촬영이 유일한 길입니다.** 네 개 PDF 를 전수 확인했고 1920px 이상은 스톡/합성 그래픽뿐, 자사 실사진 최대치는 1400×1050 ② 공장 공정 5칸(회사소개서엔 채널문자 제작 1장뿐) ③ 남은 실적 4건(교보생명·우리은행·KT탄방타워는 회사소개서에 4장이 한 띠로 합쳐져 435×276 밖에 안 나오고, 뉴코아는 사진 자체가 없음) ④ 업무 프로세스 5칸(`step-*.jpg` — **2026-08-07 부터 이걸 쓰는 화면이 하나도 없습니다.** 홈에 이어 `/process` 의 사진 칸도 걷어냈습니다. 파일을 넣어도 안 나오니, 사진이 생기면 `app/process/page.tsx` 의 `Img` 블록을 git 이력에서 먼저 되살려야 합니다). **2026-08-07 부터 빈 자리가 홈·`/process` 어디에도 안 보입니다** — 사진이 없는 구역을 화면에서 내렸기 때문입니다(§2 트리). 사진이 사라진 게 아니라 **아쉬움이 덜 급해졌을 뿐**이고, 들어오면 화면을 되살리는 게 이 줄의 목적입니다 | `public/images/README.md` |
...
| ⏳ 등록 | **구글 비즈니스 프로필 — 소유권 주장 진행중** (2026-08-06). 구글 지도에 **남이 만든 미소유 등록이 이미 있었고 내용이 틀렸습니다** — `그래픽 디자이너 / 유성구 원내동 98-1`. 주장 절차에서 **카테고리 → `간판제작업체`, 주소 → `대전 서구 사기점골길 128`, 전화 → `010-7449-4600`** 으로 교정했습니다. 인증 후 사진 22장·영업시간·홈페이지·서비스지역을 채우면 검색 우측 지식 패널이 생깁니다. ⚠️ **인증이 지금 어디까지 왔는지는 여기서 관리하지 않습니다** — 채널 상태의 최신값은 형제 저장소 `susanna` 도메인의 `notes.md` 입니다([`CLAUDE.md`](../CLAUDE.md)). 이 줄에 진행 날짜를 덧붙이면 양쪽이 반드시 어긋납니다 | [`SEO.md`](SEO.md) C2 · **형제 저장소 `notes.md`** |
...
| 구분 | 내용 | 위치 |
|---|---|---|
| 결정 | **색 명암비 미달을 알고 그대로 둡니다** (2026-08-14). `brand` 2.99:1 · `accent`(밝은 배경) 3.14:1 · placeholder 2.50:1 로 전부 WCAG AA 미달입니다. 통과값(`#007e77`/`#c64500`)으로 바꿔 **양쪽 화면을 만들어 비교한 뒤 대표님이 원래 색을 선택**했습니다 — 브랜드 톤이 눈에 띄게 탁해졌기 때문입니다. **이 줄은 "남은 일"이 아니라 "결정"입니다.** 다시 제안하기 전에 F12 의 표를 읽으세요(통과값과 구한 방법이 있습니다). 뒤집을 만한 계기: 관공서·공공기관 발주에서 웹 접근성을 요구받는 경우 | F12 · `app/globals.css` |
| 🔴 사진 | **히어로 원본 사진이 저해상도입니다 — 첫 슬라이드가 848×503 입니다** (2026-08-14 운영 실측). 화면에서 그 자리는 1813×1476 이라 **2.9배 확대**되어 나갑니다. 나머지 셋은 가로 685px 이고 그중 하나는 685×685 정사각형이라 위아래가 크게 잘립니다. 파일 자체는 무겁습니다(첫 장 **3.0MB PNG**) — **용량과 해상도는 다른 말입니다.** 이미지 최적화기에 `w=3840` 을 요청해도 원본이 작으면 **에러 없이 848px 을 그대로 돌려주므로** 경고가 없습니다. 간판 회사 첫 화면이라 여기가 가장 아픕니다. **대체 사진이 없어 지금은 못 고칩니다**(§7.1 사진 줄과 같은 원인) — 현장 촬영본 가로 2560px 급 16:9 가 들어오면 CMS(F18)로 갈아 끼우면 됩니다 | `/admin/hero` · F1 |
| 미검증 | **모바일 실기기에서 안 봤습니다** (2026-08-14). 접근성 수정(안전영역·터치 크기·라벨)은 전부 데스크톱 브라우저 폭 1813 에서만 쟀습니다. 지역 업체 트래픽은 폰에서 오므로 여기가 정작 중요합니다. 특히 `env(safe-area-inset-bottom)` 은 **아이폰 실기기에서만** 0 이 아닙니다 | F12 |
| 미검증 | 관리자 CRUD 중 **삭제 · 순서변경(`moveSlide`/`moveWork`) 이 실동작 미확인**. 저장·사진업로드는 운영에서 끝까지 통하는 것을 확인했습니다 | F9 F10 |
| 미도입 | **히어로 휠 글라이드(F1)를 그대로 뒀습니다.** 첫 화면 맨 위에서 아래로 굴리면 다음 구역까지 자동으로 미끄러집니다. 접근성 지침은 이런 스크롤 개입을 «멀미 유발»로 분류하지만, 이 구현은 발동 조건이 촘촘하고(위로 굴리는 중 미개입 · 상단 80px 안 · `prefers-reduced-motion` 이면 해제) 회귀 테스트 10건이 붙어 있어 **제품 결정 사항으로 남깁니다.** 걷어내려면 `HeroSlider` 의 `onWheel` 등록 한 곳입니다 | F1 |
...
| 부채 | **방문마다 SSR + DB 조회** — ISR 을 걷어낸 대가(§2.1 주석). 홈 HTML 에 캐시가 없습니다(`no-store` 실측). **2026-08-06 F19 로 `/about` `/signs` `/process` 도 여기 들어왔습니다** — 정적이던 세 페이지가 요청 시 SSR 이 됐습니다(문구를 관리자가 바꾸려면 불가피). **2026-08-17 F23 으로 나머지 다섯 장(`/support` `/quote` `/privacy` `/terms` `/no-email-collect`)까지 들어왔습니다** — 이제 라우트 핸들러 셋을 빼면 정적 페이지가 하나도 없습니다. 트래픽이 커지면 Cache Rules 또는 R2 로 ISR 을 되살리는 게 정석 | `app/layout.tsx` · 공개 페이지 전부 |
| 🔴 부채 | **캐시를 켜는 순간 새 디자인이 손님에게 샐 수 있습니다** (2026-08-17 F23 이 만든 부채). 위 SSR 부채를 Cache Rules·R2 로 갚을 때 **`susanna-preview` 쿠키가 붙은 요청을 우회(bypass)** 시키지 않으면, 대표님이 보던 화면이 캐시에 담겨 그대로 나갑니다. 지금은 공개 HTML 이 `no-store` 라 **터지지 않습니다** — 즉 이 둘은 **같이 갚아야 하는 한 쌍**이고, 위 줄만 보고 캐시를 켜면 사고가 납니다 | §0 A3 예외 · F23 · `lib/preview.ts` |
...
| 부채 | **`0008_product.sql` 을 아직 안 돌렸습니다** (2026-08-17). 안 돌려도 화면은 config 폴백으로 정상이지만, **돌리기 전에는 관리자 화면 "제품" 탭에서 고쳐도 화면이 안 바뀝니다** — F19 의 "구역을 비울 수 없다" 와 같은 계열의 혼란입니다 | F24 · §5 |
