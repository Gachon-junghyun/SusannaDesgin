...
| 사진 | ⚠️ 실사진 **21장**(실적 17 · 사업영역 4 · 공정 1 · **공장/사옥 2**, 일부 공용). 2026-08-07 홈의 `about-factory.jpg` 를 **제작+시공 대각 합성 사진**으로 갈았습니다 — 이 파일만 **1600×1200**(다른 자리는 1200×900). 사옥 외관 1장을 두 자리에 돌려쓰던 상태는 풀린 채입니다(`/about` 의 `about-office.jpg` 는 그대로). 남은 자리는 §7 |
...
> | 자리 | 어떻게 | 되살리는 법 |
> |---|---|---|
> | PROCESS 5칸의 사진 | `<Img>` 블록 **삭제** | `app/process/page.tsx` 의 같은 블록을 가져다 붙입니다 |
> | FABRICATION 구역 전체 | `SHOW_HOME_FABRICATION` **스위치로 차단** | 그 값을 `true` 로 |
>
> ⚠️ **DB·관리자 화면은 그대로입니다.** `/admin/content` 의 "업무 프로세스 → 사진"
> 칸과 "제작 공정" 탭이 계속 보이는데, 거기서 무엇을 고쳐도 **홈은 안 바뀝니다.**
> 사진은 `/process`, 제작 공정은 `/about#equipment` 에 여전히 나가므로 그쪽에는
> 반영됩니다. F19 의 "구역을 비울 수 없다" 함정과 같은 계열이니 헷갈리지 마세요.
...
| 구분 | 내용 | 위치 |
|---|---|---|
| 🔴 사진 | **남은 자리: 실적 4자리 + 히어로 2 + 공정 5.** 회사소개서에서 12장을 뽑아 넣어 실적 15자리 중 11자리·사업영역 4자리 전부가 채워졌고(2026-07-27), **사옥 전경은 대표님이 주신 현장 촬영본으로 채웠습니다**(2026-08-06). 급한 순서: ① **히어로 2·3 — 현장 촬영이 유일한 길입니다.** 네 개 PDF 를 전수 확인했고 1920px 이상은 스톡/합성 그래픽뿐, 자사 실사진 최대치는 1400×1050 ② 공장 공정 5칸(회사소개서엔 채널문자 제작 1장뿐) ③ 남은 실적 4건(교보생명·우리은행·KT탄방타워는 회사소개서에 4장이 한 띠로 합쳐져 435×276 밖에 안 나오고, 뉴코아는 사진 자체가 없음) ④ 업무 프로세스 5칸(`step-*.jpg` — `/process` 에서만 씀). **2026-08-07 부터 빈 자리가 홈에는 안 보입니다** — 사진이 없는 두 구역을 홈에서 내렸기 때문입니다(§2 트리). 사진이 사라진 게 아니라 **아쉬움이 덜 급해졌을 뿐**이고, 들어오면 홈을 되살리는 게 이 줄의 목적입니다 | `public/images/README.md` |
